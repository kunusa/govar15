...
class WebsiteSupportTicket(models.Model):
    ...
    def _default_state(self):
        return self.env.ref('website_support.website_ticket_state_open').mail_template_id
    ...
    channel = fields.Char(string="Canal", default="Manual")
    ...
    parent_company_id = fields.Many2one(string="Compañia", related="partner_id.company_id")
    partner_id = fields.Many2one('res.partner', string="Cliente")
    user_id = fields.Many2one('res.users', string="Usuario asignado")
    person_name = fields.Char(string='Nombre')
    email = fields.Char(string="Email")
    support_email = fields.Char(string="Correo de soporte")
    category_id = fields.Many2one('website.support.ticket.category', string="Categoria", track_visibility='onchange')
    ...
    state_id = fields.Many2one('website.support.ticket.state', group_expand='_read_group_state', default=_default_state,
                            string="Estado")
    conversation_history_ids = fields.One2many('website.support.ticket.message', 'ticket_id', string="Historail de conversacion")
    ...
    ticket_number = fields.Char(string="Numero de ticket", readonly=True)
    ticket_color = fields.Char(related="priority_id.color", string="Color de ticket")
    company_id = fields.Many2one('res.company', string="Compañia",
                                 default=lambda self: self.env['res.company']._company_default_get('website.support.ticket') )
    support_rating = fields.Integer(string="Calificación de soporte")
    support_comment = fields.Text(string="Comentario de soporte")
    close_comment = fields.Html(string="Comentario de cierre")
    close_time = fields.Datetime(string="Hora de cierre")
    close_date = fields.Date(string="Fecha de cierra")
    closed_by_id = fields.Many2one('res.users', string="Cerrado por")
    time_to_close = fields.Integer(string="Tiempo de cierre (segundos)")
    extra_field_ids = fields.One2many('website.support.ticket.field', 'wst_id', string="Detalles extras")
    planned_time = fields.Datetime(string="Tiempo planificado")
    planned_time_format = fields.Char(string="Formato de timepo planificado", compute="_compute_planned_time_format")
    approval_id = fields.Many2one('website.support.ticket.approval', default=_default_approval_id, string="Aprovado")
    approval_message = fields.Text(string="Mensaje de aprobacion")
    approve_url = fields.Char(compute="_compute_approve_url", string="Aprobacion URL")
    disapprove_url = fields.Char(compute="_compute_disapprove_url", string="Desaprobación URL")
    tag_ids = fields.Many2many('website.support.ticket.tag', string="Etiquetas")
    sla_id = fields.Many2one('website.support.sla', string="SLA")
    sla_timer = fields.Float(string="SLA Tiempo restante")
    sla_timer_format = fields.Char(string="SLA Formato", compute="_compute_sla_timer_format")
    sla_active = fields.Boolean(string="SLA Activo")
    sla_rule_id = fields.Many2one('website.support.sla.rule', string="SLA Regla")
    ...
    def update_sla_timer(self):
        ...
        for active_sla_ticket in self.env['website.support.ticket'].search([('sla_active','=',True),('sla_id','!=',False),('sla_rule_id','!=',False)]):
            ...
            if active_sla_ticket.sla_rule_id.countdown_condition == 'business_only':
                ...
                setting_business_hours_id = int(self.env['ir.config_parameter'].sudo().get_param('website_support.business_hours_id', '0')) or False
            ...
            for sla_alert in self.env['website.support.sla.alert'].search([('vsa_id','=',active_sla_ticket.sla_id.id), ('alert_time','>=', active_sla_ticket.sla_timer)]):
                ...
                if sla_alert not in active_sla_ticket.sla_alert_ids:
                    ...
                    for my_user in active_sla_ticket.category_id.cat_user_ids:
                        ...
                        values = notification_template.generate_email([active_sla_ticket.id], fields=fields_to_generate)
                        if active_sla_ticket.id in values:
                            values = values[active_sla_ticket.id]
    ...
    def message_new(self, msg, custom_values=None):
        ...
        setting_email_default_category_id = int(self.env['ir.config_parameter'].sudo().get_param('website_support.email_default_category_id', '0')) or False
    ...
    def request_approval(self):
        ...
        fields_to_generate = ['subject', 'body', 'partner_ids']
        values = self.env['mail.compose.message'].generate_email_for_composer(approval_email.id, [self.id], fields=fields_to_generate)
    ...
    def _needaction_domain_get(self):
        open_state = self.env.ref('website_support.website_ticket_state_open').mail_template_id
    ...
    def write(self, values, context=None):
        ...
        if 'user_id' in values:
            setting_change_user_email_template_id = int(self.env['ir.config_parameter'].sudo().get_param('website_support.change_user_email_template_id', '0')) or False
            ...
            fields_to_generate = ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date']
            email_values = email_template.generate_email([self.id], fields=fields_to_generate)
            if self.id in email_values:
                email_values = email_values[self.id]
    ...
    def send_survey(self):
        ...
        fields_to_generate = ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date']
        values = notification_template.generate_email([self.id], fields=fields_to_generate)
        if self.id in values:
            values = values[self.id]
        send_mail = self.env['mail.mail'].create(values)
        send_mail.send(True)
...
class WebsiteSupportTicketMessage(models.Model):
    ...
    def create(self, values):
        ...
        if values['by'] == "customer":
            ...
            fields_to_generate = ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date']
            email_values = customer_reply_email_template.generate_email([new_record.id], fields=fields_to_generate)
            if new_record.id in email_values:
                email_values = email_values[new_record.id]
...
class WebsiteSupportTicketCategory(models.Model):
    ...
    name = fields.Char(required=True, translate=True, string='Nombre de categoria')
    cat_user_ids = fields.Many2many('res.users', string="Categoria de usuarios")
    access_group_ids = fields.Many2many('res.groups', string="Grupos de acceso", help="Restrict which users can select the category on the website form, none = everyone")
...
class WebsiteSupportTicketClose(models.TransientModel):
    ...
    def _onchange_template_id(self):
        if self.template_id:
            fields_to_generate = ['subject', 'body', 'partner_ids']
            values = self.env['mail.compose.message'].generate_email_for_composer(self.template_id.id, [self.ticket_id.id], fields=fields_to_generate)
            if self.ticket_id.id in values:
                values = values[self.ticket_id.id]
    ...
    def close_ticket(self):
        ...
        setting_auto_send_survey = self.env['ir.config_parameter'].sudo().get_param('website_support.auto_send_survey', 'False') == 'True'
...
class WebsiteSupportTicketCompose(models.Model):
    ...
    def send_reply(self):
        ...
        #Send email
        setting_staff_reply_email_template_id = int(self.env['ir.config_parameter'].sudo().get_param('website_support.staff_reply_email_template_id', '0')) or False
        ...
        if setting_staff_reply_email_template_id:
            ...
            # Usar send_mail que maneja correctamente el renderizado y el envío
            # force_send=True envía inmediatamente, email_values permite cambiar el modelo/res_id del mensaje en chatter
            email_wrapper.send_mail(
                self.id, 
                force_send=True,
                email_values={
