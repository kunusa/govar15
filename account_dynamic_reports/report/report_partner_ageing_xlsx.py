...
class InsPartnerAgeingXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            # Date from
            self.sheet_2.write_string(self.row_pos_2, 0, _('A la fecha'), #As on Date
                                    self.format_header)
            ...
            # Partners
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Clientes/proveedore'),
                                                 self.format_header)
    ...
    def prepare_report_contents(self, data, period_dict, period_list, ageing_lines, filter):
        ...
        if self.record.include_details:
            self.sheet.write_string(self.row_pos, 0,  _('Entrada #'), self.format_header)
            self.sheet.write_string(self.row_pos, 1, _('Fecha de vencimiento'), self.format_header) #Due Date
            self.sheet.write_string(self.row_pos, 2, _('Diario'), self.format_header)
            self.sheet.write_string(self.row_pos, 3, _('Cuenta'), self.format_header)
        else:
            self.sheet.merge_range(self.row_pos, 0, self.row_pos, 3, _('Cliente/proveedor'),
                                   self.format_header)
        k = 4
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet = workbook.add_worksheet('Antiguedad de Saldos')
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(0, 0, 0, 11, 'Antiguedad de Saldos'+' - '+data[0]['company_id'][1], self.format_title)
