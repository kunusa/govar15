...
class KsSaleOrderInherit(models.Model):
    ...
    ks_sync_states = fields.Boolean(string="Sync Status",store=True,compute='compute_sync_status', readonly=True)
    ...
    def ks_woo_import_order_create(self, order_data, instance, queue_record=False):
        try:
            ...
            if e.args[0] == "'bool' object has no attribute 'id'":
                e = "Woo Record was excepted either Actual, Shipping, Billing Address"
            if queue_record:
    ...
    def ks_woo_import_order_update(self, order_data, queue_record=False, update=False):
        try:
            if self.state in ["draft", "sent", "cancel"]:
                ...
                if order_json:
                    order_lines_json = order_json.get('order_line', [])
                    if 'order_line' in order_json:
                        del order_json['order_line']
                    self.write(order_json)
                    self.order_line = order_lines_json
                    updated_list = [i[2]['ks_woo_order_line_id'] for i in order_lines_json]
                    order_line_list = [i['ks_woo_order_line_id'] for i in self.order_line]
                    order_line_diff_ids = list(set(order_line_list).difference(updated_list))
                    if order_line_diff_ids:
                        remove_ids = self.order_line.search([('ks_woo_order_line_id', '=', order_line_diff_ids)]).ids
                        self.update({'order_line': [(2, remove_id, 0) for remove_id in remove_ids]})
        ...
        if tax:
            ...
            for ol_tax in order_line_tax:
                for each_record in tax:
                    total_tax = ol_tax['total']
                    if total_tax:
                        if ol_tax.get('id') == each_record.get('rate_id'):
