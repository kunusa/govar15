...
class KsResPartnerInherit(models.Model):
    ...
    def ks_map_odoo_partner_data_to_create(self, json_data, odoo_partner=False, instance=False):
        ...
        if instance.ks_customer_company_type == True:
            data = {
                'name': json_data.get('billing').get('company') or json_data.get('shipping').get('company'),
                'company_type': 'company' if json_data.get('billing').get('company') or json_data.get('shipping').get(
                    'company') else 'person'
            }
        else:
            data = {
