...
class KsWooProductVariant(models.Model):
            ...
            if odoo_product and layer_product:
                ...
                for variant in variants:
                    ...
                    if variant_prepared:
                        ...
                        self.env['product.product'].ks_manage_price_to_export(variant,
                                                                              instance)
                    # Manejar imágenes de variantes - similar al template
                    # Primero verificar si hay imágenes en ks_image_ids
                    if variant.ks_image_ids:
                        for image in variant.ks_image_ids:
                            image_id = self.env['ks.woo.product.images'].ks_odoo_prepare_image_data(image,
                                                                                                    template_id=variant_prepared.ks_wc_product_tmpl_id.id,
                                                                                                    variant_id=variant_prepared.id)
                    
                    # Si hay image_1920 pero no está procesada en WooCommerce, asegurar que se procese
                    if variant.image_1920:
                        # Verificar si ya existe una imagen procesada en WooCommerce para esta variante
                        has_processed_images = variant_prepared.ks_wc_image_ids.filtered(
                            lambda img: img.ks_wc_variant_id == variant_prepared.id
                        )
                        
                        # Si no hay imágenes procesadas, crear/actualizar la imagen principal
                        if not has_processed_images:
                            # Buscar si existe un registro en ks.common.product.images para este variant
                            # que pueda corresponder a image_1920
                            existing_common_image = variant.ks_image_ids[:1] if variant.ks_image_ids else None
                            
                            if not existing_common_image:
                                # Crear un nuevo registro en ks.common.product.images para image_1920
                                image_values = {
                                    "sequence": 0,
                                    "ks_image": variant.image_1920,
                                    "ks_name": variant.name,
                                    "ks_variant_id": variant.id,
                                    "ks_template_id": variant.product_tmpl_id.id
                                }
                                common_image = self.env["ks.common.product.images"].with_context(main_image=True).create(image_values)
                                # Procesar la imagen para WooCommerce usando el método estándar
                                image_id = self.env['ks.woo.product.images'].ks_odoo_prepare_image_data(common_image,
                                                                                                        template_id=variant_prepared.ks_wc_product_tmpl_id.id,
                                                                                                        variant_id=variant_prepared.id)
                            else:
                                # Si existe ks_image_ids, ya se procesaron arriba
                                # Pero si image_1920 no está en ks_image_ids, crear registro y procesarlo
                                # Usar la primera imagen disponible o crear nueva si es necesario
                                image_id = self.env['ks.woo.product.images'].ks_odoo_prepare_image_data(existing_common_image,
                                                                                                        template_id=variant_prepared.ks_wc_product_tmpl_id.id,
                                                                                                        variant_id=variant_prepared.id)
        except Exception as e:
            raise e
    ...
    def ks_map_variant_data_for_layer(self, variant, all_variants_data, instance, woo_layer_product):
        ...
        """
        translated_desc = self.env['ir.translation'].search([('src','=',variant.get("description")),('lang','=',instance.ks_language.code)], limit=1)
        if not variant.get("description") and not translated_desc:
            translated_desc = variant.get("description")
        else:
            if translated_desc:
                ...
                translated_desc = self.env['ir.translation'].create({
                    'src':variant.get('description'),
                    'value':variant.get('description'),
                    'lang':instance.ks_language.code,
                    'name': 'ks.woo.product.variant,ks_woo_description',
                    'type': 'model',
                    'state':'translated',
                })
            translated_desc = translated_desc.value
        data = {
