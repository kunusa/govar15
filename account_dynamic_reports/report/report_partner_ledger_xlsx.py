...
class InsPartnerLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            # Partners
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Cliente/proveedors'),
                                                 self.format_header)
            ...
            # Partner Tags
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Etiqueta de Cliente/proveedor'),
                                      self.format_header)
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.write_string(self.row_pos, 1, _('JRNL'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 2, _('Cliente/proveedor'),
                                    self.format_header)
            ...
            self.sheet.write_string(self.row_pos, 4, _('Etiqueta de entrada'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Débito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Crédito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 7, _('Balance'),
                                    self.format_header)
        else:
            self.sheet.merge_range(self.row_pos, 0, self.row_pos, 4, _('Cliente/proveedor'), self.format_header)
            self.sheet.write_string(self.row_pos, 5, _('Débito'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 6, _('Crédito'),
                                    self.format_header)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet = workbook.add_worksheet('Estado de cuenta')
        self.sheet_2 = workbook.add_worksheet('Filtros')
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(0, 0, 0, 8, 'Estado de cuenta'+' - '+data[0]['company_id'][1], self.format_title)
