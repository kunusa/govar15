...
class InsTrialBalance(models.TransientModel):
    ...
    show_hierarchy = fields.Boolean(
        string='Mostar jerarquía'
    )
    target_moves = fields.Selection(
        [('all_entries', 'Todos'),
         ('posted_only', 'contabilizados')], string='Movimientos',
        default='posted_only', required=True
    )
    display_accounts = fields.Selection(
        [('all', 'Todas'),
         ('balance_not_zero', 'Balance diferente a 0')], string='Mostar Cuentas',
    ...
    analytic_ids = fields.Many2many(
        'account.analytic.account', string='Cuentas analícticas'
    )
    ...
    def validate_data(self):
        if self.date_from > self.date_to:
            raise ValidationError(_('"Desde" debe ser menor o igual a "Hasta"'))
        return True
    ...
    def process_filters(self, data):
        ...
        if data.get('date_from') > data.get('date_to'):
            raise ValidationError(_('"Desde" no debe ser menor a "fecha"'))

        if not data.get('date_from') or not data.get('date_to'):
            raise ValidationError(_('"Desde" and "Hasta" son obligatorias para este reporte'))
        ...
        if data.get('analytic_ids', []):
            ...
            filters['analytics'] = ['Todas']

        if data.get('display_accounts') == 'all':
            filters['display_accounts'] = 'Todas'
        else:
            filters['display_accounts'] = 'Con balance diferente a 0'
    ...
    def process_data(self, data):
        ...
        if data:
            ...
            if periodo:
                WHERE += f' AND m.period_id != {periodo}'
            ...
            for account in account_ids:
                ...
                # if fiscal_year_id:
                #     WHERE_INIT += f' AND m.fiscal_year = {test}'
                if periodo:
                    WHERE_INIT += f' AND m.period_id != {periodo}'
                ...
                sql = ('''
                    ...
                    JOIN sh_fiscal_year sfy on m.fiscal_year = sfy.id 
                    join sh_account_period sap  on m.period_id = sap.id
                    WHERE %s
                ''')%WHERE_INIT
                cr.execute(sql)
                ...
                if fiscal_year_id:
                    WHERE_CURRENT += f' AND m.fiscal_year = {test}'
                sql = ('''
                    SELECT
    ...
    def get_start_day_month(self, date):
        date = date - timedelta(days=(date.day - 1 ))
        return date

    def get_end_day_month(self, start_date, month):
    ...
    def create_normal(self,):
        ...
        return self.action_xml(aditonal_info)
    ...
    def create_complementary(self,):
        ...
        aditonal_info = {
            "TipoEnvio": "C",
            "FechaModBal": fecha_formateada
        }
        return self.action_xml(aditonal_info)
    
    def action_xml(self, type_data):
        filters, account_lines, retained, subtotal = self.get_report_datas()
        start_date = self.get_start_day_month(filters['date_from']) 
        end_date = self.get_end_day_month(start_date,int(datetime.strftime(start_date, "%d")))
        if start_date != filters['date_from'] or end_date != filters['date_to']:
            raise UserError(f'Para Generar el XML el filtro debe estar en el primer día y en último de un mismo mes. favor de modificar el filtro: por ejemplo {start_date} y {end_date}')
        list_trial_balance = []
        aditonal_info = []
        if account_lines:
            for clave, valor in account_lines.items():

                trial_balance = {
                    "SaldoIni": valor['initial_balance'] or 0,
                    "Haber" : valor['credit'] or 0,
                    "Debe": valor['debit'] or 0,
                    "SaldoFin": valor['ending_balance'] or 0,
                    "NumCta": clave
                }
                list_trial_balance.append(trial_balance)
        aditonal_info = {
            "RFC" : self.env.user.company_id.company_registry,
            "Anio": str(start_date.year),
            "Mes" : str(start_date.month),
        }
        if start_date.month < 10:
            aditonal_info["Mes"] = f"0{start_date.month}"
        aditonal_info.update(type_data)
        list_trial_balance.append(aditonal_info)

        json_data = json.dumps(list_trial_balance)
        save_path_file = "{}{}{}B{}.xml".format(self.env.user.company_id.company_registry,str(start_date.year),str(aditonal_info["Mes"]),aditonal_info['TipoEnvio'])
        conf = self.env['api.configuration'].sudo().search([('id', '!=', 0)],limit=1)
        conf.validar_usuario()
        data = self.env['app.helpers'].get_trial_balance_xml(conf.token,json_data,conf.url) 
        return self.env['app.helpers'].download_trial_balance(data,save_path_file)
    ...
    def action_xlsx(self):
        ...
        # Write filters
        row_pos_2 += 2
        sheet_2.write(row_pos_2, 0, _('Desde'), format_header)
        ...
        sheet.write(row_pos, 5, _(' Para '), format_merged_header_without_border)
        ...
        sheet.merge_range(row_pos, 7, row_pos, 9, 'Balance final', format_merged_header)
        row_pos += 1
        ...
        sheet.write(row_pos, 4, _('Débito'), format_header)
        sheet.write(row_pos, 5, _('Crédito'), format_header)
        sheet.write(row_pos, 6, _('Balance'), format_header)
        sheet.write(row_pos, 7, _('Débito'), format_header)
        sheet.write(row_pos, 8, _('Crédito'), format_header)
        ...
        report_id = self.env['common.xlsx.out'].sudo().create({'filedata': result, 'filename': 'BalanzadeComprobacion.xls'})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xls' % (
                report_id.id, 'Balanza de Comprobación.xls'),
            'target': 'new',
        }
