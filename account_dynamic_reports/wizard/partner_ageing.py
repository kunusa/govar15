...
class InsPartnerAgeing(models.TransientModel):
    ...
    as_on_date = fields.Date(string='A la fecha', required=True, default=fields.Date.today()) #As on Date
    ...
    type = fields.Selection([('receivable','Solo cuentas por cobrar'),
                              ('payable','Solo cuentas por pagar')], string='Tipo')
    partner_type = fields.Selection([('customer', 'Solo clientes'),
                             ('supplier', 'Solo proveedores')], string='Tipo de Cliente/proveedor')

    partner_ids = fields.Many2many(
        'res.partner', string='Clientes/proveedores', required=False
    )
    ...
    def validate_data(self):
        if not(self.bucket_1 < self.bucket_2 and self.bucket_2 < self.bucket_3 and self.bucket_3 < self.bucket_4 and \
            ...
            raise ValidationError(_('"El orden debe ser ascendente"'))#Bucket order must be ascending
        return True
        ...
        if partner:
            ...
            sql = """
                    ...
                    WHERE
                        ...
                        AND j.type in ('sale', 'purchase')
                    GROUP BY
                        l.date,m.ref,l.date_maturity, m.id, m.name, j.name, a.name, cc.id
                    OFFSET %s ROWS
    ...
    def action_xlsx(self):
        ...
        sheet = workbook.add_worksheet('Antiguedad de saldos')
        sheet.set_zoom(95)
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 11, 'Antiguedad de saldos' + ' - ' + data['company_id'][1], format_title)

        # Write filters
        row_pos_2 += 2
        sheet_2.write(row_pos_2, 0, _('A la fecha'), format_header) #As on Date
        ...
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Cliente/proveedor'), format_header)
        ...
        if record.include_details:
            sheet.write(row_pos, 0,  _('Entrada #'), format_header)
            sheet.write(row_pos, 1, _('Fecha de vencimiento'), format_header)
            sheet.write(row_pos, 2, _('Diario'), format_header)
            sheet.write(row_pos, 3, _('Cuenta'), format_header)
        else:
            sheet.merge_range(row_pos, 0, row_pos, 3, _('Cliente/proveedor'),
                                   format_header)
        k = 4
        ...
        report_id = self.env['common.xlsx.out'].sudo().create({'filedata': result, 'filename': 'AntiguedadDeSaldos.xls'})
        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xls' % (
                report_id.id, 'Antiguedad de saldos.xls'),
            'target': 'new',
        }
