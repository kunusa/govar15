...
class InsGeneralLedger(models.TransientModel):
    ...
    sort_accounts_by = fields.Selection(
        [('date', 'Fecha'), ('journal', 'Diario y socio')], string='Ordenar por',
        default='date', required=True
    )
    display_accounts = fields.Selection(
        [('all', 'Todas'),
         ('balance_not_zero', 'Balance diferente a 0')], string='Mostrar cuentas',
    ...
    partner_ids = fields.Many2many(
        'res.partner', string='Clientes/proveedores'
    )
    ...
    def process_filters(self):
        ...
        if data.get('sort_accounts_by', False) == 'date':
            filters['sort_accounts_by'] = 'Fecha'
        else:
            filters['sort_accounts_by'] = 'Diario y socio'
    ...
    def action_xlsx(self):
        ...
        sheet.set_zoom(95)
        sheet_2 = workbook.add_worksheet('Filtros')
        sheet_2.protect()
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'Libro Mayor' + ' - ' + data['company_id'][1], format_title)
        ...
        sheet_2.write(row_pos_2, 1, datestring or '',content_header_date)
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Movimientos'),format_header)
        ...
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Ordenar por'),format_header)
        ...
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Clientes/proveedores'),format_header)
        ...
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Etiquetas de cuenta'),format_header)
        ...
        sheet_2.write(row_pos_2, 1, a_list,content_header)
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Etiquetas analíticas'),format_header)
        ...
        if filter.get('include_details', False):
            sheet.write_string(row_pos, 0, _('Fecha'), format_header)
            sheet.write_string(row_pos, 1, _('JRNL'), format_header)
            sheet.write_string(row_pos, 2, _('Cliente/proveedor'), format_header)
            # self.sheet.write_string(row_pos, 3, _('Ref'),self.format_header)
            sheet.write_string(row_pos, 3, _('Movimiento'), format_header)
            sheet.write_string(row_pos, 4, _('Etiqueta de la entrada'), format_header)
            sheet.write_string(row_pos, 5, _('Débito'), format_header)
            sheet.write_string(row_pos, 6, _('Crédito'), format_header)
            sheet.write_string(row_pos, 7, _('Balance'), format_header)
        else:
            sheet.merge_range(row_pos, 0, row_pos, 1, _('Código'), format_header)
            sheet.merge_range(row_pos, 2, row_pos, 4, _('Cuenta'), format_header)
            sheet.write_string(row_pos, 5, _('Débito'), format_header)
            sheet.write_string(row_pos, 6, _('Crédito'), format_header)
            sheet.write_string(row_pos, 7, _('Balance'), format_header)
        ...
        report_id = self.env['common.xlsx.out'].sudo().create({'filedata': result, 'filename': 'LibroMayor.xls'})
        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xls' % (
            report_id.id, 'Libro Mayor.xls'),
            'target': 'new',
        }
