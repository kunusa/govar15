...
class InsFinancialReport(models.TransientModel):
    ...
    def _compute_account_balance(self, accounts, report,type_report,cmp_dates):
        ...
        if accounts:
            if self.account_report_id != \
                ...
                if report.type in ['accounts','account_type'] and report.range_selection == 'current_date_range':
                    if self.date_to and self.date_from:
                        if type_report == 'balance_general':
                            context.update({'strict_range': True, 'initial_bal': False, 'date_from': self.date_from,'date_to': self.date_to})
                        if type_report != 'balance_general' and cmp_dates == True:
                            context.update({'strict_range': True, 'initial_bal': False, 'date_from': self.date_from,'date_to': self.date_to})
                        if type_report != 'balance_general' and cmp_dates == False:
                            context.update({'strict_range': True, 'initial_bal': False, 'date_from': self.date_from_cmp,'date_to': self.date_to_cmp})
                ...
                if type_report == 'balance_general':
                    if self.date_to and self.date_from:
                        where_clause = re.sub("< ", "<=", where_clause, count=1)
                        where_params[0] = self.date_to.strftime("%Y-%m-%d")
                    else:
                        raise UserError(_('From date and To date are mandatory to generate this report'))   

                if type_report == 'period_general_balance':
                    if self.date_to and self.date_from:
                        where_clause = re.sub("< ", "<=", where_clause, count=1)
                        where_params[0] = self.date_to.strftime("%Y-%m-%d")
                        tables += """
                        JOIN account_move am on account_move_line.move_id = am.id 
                        JOIN sh_fiscal_year sfy on am.fiscal_year = sfy.id 
                        join sh_account_period sap  on am.period_id = sap.id
                        """
                    else:
                        raise UserError(_('From date and To date are mandatory to generate this report')) 
            ...
            fiscal_year_id = self.env['sh.fiscal.year'].search([('date_start', '=', self.date_from),('date_end', '=', self.date_to)])
            test = fiscal_year_id.id
            periodo = False
            period_obj = self.env['sh.account.period']
            record = period_obj.search([('special', '=', True),('fiscal_year_id','=',test)],limit=1)
            if record:
                month, year =  record.code.split('/')
                next_year = str(int(year) + 1)
                next_value = f"{month}/{next_year}"
                period_id = period_obj.search([('special', '=', True),('code','=',next_value)], limit=1)
                if period_id:
                    periodo = period_id.id
            ...
            if periodo:
                request = "SELECT account_id as id, " + ', '.join(mapping.values()) + \
                       " FROM " + tables + \
                        " WHERE account_id IN %s " + f"""AND ("account_move_line"."period_id" != {periodo})""" \
                            + filters + \
                       " GROUP BY account_id"
    ...
    def _compute_report_balance(self, reports,report_type,cmp_dates):
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                if self.account_report_id != \
                    ...
                    res[report.id]['account'] = self._compute_account_balance(report.account_ids, report,report_type,cmp_dates)
                    for value in res[report.id]['account'].values():
                        for field in fields:
                            res[report.id][field] += value.get(field)
                else:
                    res2 = self._compute_report_balance(report.parent_id,report_type,cmp_dates)
                ...
                if self.account_report_id != \
                    ...
                    res[report.id]['account'] = self._compute_account_balance(accounts, report,report_type,cmp_dates)
                    for value in res[report.id]['account'].values():
                        for field in fields:
                            res[report.id][field] += value.get(field)
                else:
                    ...
                    res[report.id]['account'] = self._compute_account_balance(
                        accounts, report, report_type,cmp_dates)
                    for value in res[report.id]['account'].values():
                        for field in fields:
                            res[report.id][field] += value.get(field)
                ...
                if self.account_report_id != \
                        self.env.ref('account_dynamic_reports.ins_account_financial_report_cash_flow0'):
                    res2 = self._compute_report_balance(report.account_report_id,report_type,cmp_dates)
                    for key, value in res2.items():
                        for field in fields:
                            res[report.id][field] += value[field]
                else:
                    res[report.id]['account'] = self._compute_account_balance(
                        report.account_ids, report, report_type,cmp_dates)
                    for value in res[report.id]['account'].values():
                        for field in fields:
                            res[report.id][field] += value.get(field)
                ...
                if self.account_report_id != \
                        self.env.ref('account_dynamic_reports.ins_account_financial_report_cash_flow0'):
                    res2 = self._compute_report_balance(report.children_ids,report_type,cmp_dates)
                    for key, value in res2.items():
                        for field in fields:
                            res[report.id][field] += value[field]
                else:
    ...
    def get_account_lines(self, data):
        ...
        current_balance = 0.0
        ending_balance = 0.0
        type_report = self.account_report_id.range_selection
        account_report = self.account_report_id
        child_reports = account_report._get_children_by_order(strict_range = self.strict_range)
        res = self.with_context(data.get('used_context'))._compute_report_balance(child_reports,type_report,True)
        if self.account_report_id == \
                self.env.ref('account_dynamic_reports.ins_account_financial_report_cash_flow0'):
            ...
            initial_balance = self.with_context(cashflow_context)._compute_report_balance(child_reports,type_report,True). \
        ...
        if data['enable_filter']:
            comparison_res = self.with_context(data.get('comparison_context'))._compute_report_balance(child_reports,type_report,False)
    ...
    def get_report_values(self):
        ...
        data['form'] = self.read(
            ['date_from', 'enable_filter', 'debit_credit', 'date_to', 'date_range',
             ...
             'company_id','enable_filter','date_from_cmp','date_to_cmp','label_filter','filter_cmp','label_month','x_report_type'])[0]
        ...
        if self.account_report_id == \
            ...
            if self.strict_range:
                ...
                data['form']['rtype'] = 'PANDL'
        # se hacen modificaciones en el envio de datos para el tipo cmpq
        if self.x_report_type:
            activos = []
            pasivo_capital = []
            total =[]
            current_group = None
            totalp = False
            totalc = False
            for item in data['report_lines']:
                if item['name'] == 'Total Activos':
                    total.append(item)
                    current_group = 'activos'
                    continue
                elif item['name'] == 'Total Pasivo y Capital':
                    total.append(item)
                    current_group = 'pasivo_capital'
                    continue
                elif item['name'] == 'Balance General':
                    total.append(item)
                    continue

                if current_group == 'activos':
                    activos.append(item)
                elif current_group == 'pasivo_capital':
                    pasivo_capital.append(item)

            for i, rec in enumerate(pasivo_capital):
                if rec['name'] == 'Total Pasivos':
                    totalp = rec
                    del pasivo_capital[i] 
                elif rec['name'] == 'Total Capital':
                    totalc = rec
                    pasivo_capital[i] = totalp
            pasivo_capital.append(totalc)
            data['report_lines_total'] = total
            data['report_lines'] = activos
            data['report_lines_pasivo'] = pasivo_capital
        return data
    ...
    financial_year = fields.Selection(
        ...
        string='Año fiscal', default=lambda self: self.env.company.financial_year, required=True)
    ...
    target_move = fields.Selection([('posted', 'Contabilizados'),
                                    ('all', 'Todos'),
    ...
    debit_credit = fields.Boolean(
        string='Mostrar columnas crédito/débito',
        default=True,
        help="Ayuda a identificar débito y crédito con línea de saldo para una mejor comprensión.")
    ...
    date_from_cmp = fields.Date(string='Fecha incial de comparación')
    date_to_cmp = fields.Date(string='Fecha final comparación')
    filter_cmp = fields.Selection([('filter_no', 'Sin filtro'), ('filter_date', 'Fecha')], string='Filtrar por',
                                  required=True, default='filter_date')
    label_filter = fields.Char(string='Etiqueta de columna', default='Acumulado',
                               help="Esta etiqueta se mostrará en el Reporte para mostrar el saldo calculado para el filtro de comparación dado.")
    label_month = fields.Char(string='Etiqueta de Mes', default='Mes')

    x_report_type = fields.Selection(string='Tipo de Reporte', 
                                    selection=[('cmpq', 'Horizontal'),],
                                    help="Solo llenar el Campo en Balance General")
    ...
    def action_xlsx(self):
        ...
        sheet.set_zoom(95)
        sheet2 = workbook.add_worksheet('Filtros')
        sheet2.protect()
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet2.write(row_pos_2, 0, _('Desde'), format_header)
        ...
        row_pos_2 += 1
        # Date to
        sheet2.write(row_pos_2, 0, _('Hasta'), format_header)
        ...
        if data['form']['enable_filter']:
            ...
            sheet2.write(row_pos_2, 0, _('Comparación desde'),
                                      format_header)
            ...
            row_pos_2 += 1
            # Compariosn Date to
            sheet2.write(row_pos_2, 0, _('Comparación hasta'),
                                      format_header)
        ...
        if data['form']['debit_credit'] == 1:
            ...
            sheet.write(row_pos, 0, _('Nombre'), format_header)
            sheet.write(row_pos, 1, _('Débito'), format_header)
            sheet.write(row_pos, 2, _('Crédito'), format_header)
        ...
        if data.get('initial_balance') or data.get('current_balance') or data.get('ending_balance'):
            row_pos += 2
            sheet.merge_range(row_pos, 1, row_pos, 2, 'Saldo de caja inicial', tmp_style_num) #Initial Cash Balance
            sheet.write(row_pos, 3, float(data.get('initial_balance')), tmp_style_num)
            row_pos += 1
            sheet.merge_range(row_pos, 1, row_pos, 2, 'Saldo de caja actual', tmp_style_num) #
            sheet.write(row_pos, 3, float(data.get('current_balance')), tmp_style_num)
            row_pos += 1
            sheet.merge_range(row_pos, 1, row_pos, 2, 'Saldo de caja neto', tmp_style_num) #
