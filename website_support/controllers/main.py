...
class SupportTicketController(http.Controller):
    ...
    def support_approve(self, ticket_id, **kwargs):
        ...
        if support_ticket.approval_id.id == awaiting_approval.id:
            ...
            for my_user in support_ticket.category_id.cat_user_ids:
                ...
                values = notification_template.generate_email([support_ticket.id], fields=fields_to_generate)
                if support_ticket.id in values:
                    values = values[support_ticket.id]
    ...
    def support_account_create(self, **kw):
        ...
        setting_allow_user_signup = request.env['ir.config_parameter'].sudo().get_param('website_support.allow_user_signup', 'False') == 'True'
    ...
    def support_submit_ticket(self, **kw):
        ...
        setting_google_recaptcha_active = request.env['ir.config_parameter'].sudo().get_param('website_support.google_recaptcha_active', 'False') == 'True'
        setting_google_captcha_client_key = request.env['ir.config_parameter'].sudo().get_param('website_support.google_captcha_client_key', '')
        setting_max_ticket_attachments = int(request.env['ir.config_parameter'].sudo().get_param('website_support.max_ticket_attachments', '2'))
        setting_max_ticket_attachment_filesize = int(request.env['ir.config_parameter'].sudo().get_param('website_support.max_ticket_attachment_filesize', '500'))
        setting_allow_website_priority_set = request.env['ir.config_parameter'].sudo().get_param('website_support.allow_website_priority_set', 'partner')
    ...
    def support_process_ticket(self, **kwargs):
        ...
        if setting_google_recaptcha_active:
            ...
            setting_google_captcha_secret_key = request.env['ir.config_parameter'].sudo().get_param('website_support.google_captcha_secret_key', '')
    ...
    def support_portal_ticket_comment(self, **kw):
        ...
        support_ticket.state_id = request.env.ref('website_support.website_ticket_state_customer_replied').id
    ...
    def support_ticket_comment(self, **kw):
        ...
        if ticket.partner_id.id == http.request.env.user.partner_id.id:
            ...
            ticket.state_id = request.env.ref('website_support.website_ticket_state_customer_replied').id
