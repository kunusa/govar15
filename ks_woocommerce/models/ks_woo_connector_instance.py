...
class KsWooCommerceConnectorInstance(models.Model):
    ...
    ks_is_named_guest_customer = fields.Boolean('Actual customer', help="Enables/disables - Creates customers that have billing, shipping, or actual address if enabled.")
    ...
    def ks_woo_connect_to_instance(self):
        ...
        try:
            ...
            if woo_api.get("").status_code == 200:
                ...
                self.ks_stock_field_type = self.env['ir.model.fields'].search([('name','=','free_qty'), ('model', '=', 'product.product')], limit=1).id
