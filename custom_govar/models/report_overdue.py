...
class ReportOverdue(models.AbstractModel):
    ...
    @api.model
    def render_html(self, docids, id_partner):
        totals = {}
        ...
        # partner_id = self.env['res.partner'].search([('id','=',id_partner)])
        report = self.env.ref('custom_govar.action_report_overdue')
        # self.env.ref('stock.action_report_delivery')._render_qweb_pdf(self.id)
        # report = self.env['ir.actions.report']._get_report_from_name('custom_govar.report_overdue')
        pdf_content, content_type = report._render_qweb_pdf(docids, data=docargs)
