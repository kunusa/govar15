...
class InsGeneralLedgerXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, filter):
        ...
        if filter:
            ...
            self.sheet_2.write_string(self.row_pos_2, 1, filter['display_accounts'],
                                    self.content_header)
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Ordenar por'),
                                    self.format_header)
            ...
            # Account Tags
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Etiquetas de cuenta'),
                                      self.format_header)
            ...
            # Analytic Accounts
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('Cuentas analíticas'),
                                      self.format_header)
            ...
            # Analytic Tags
            self.row_pos_2 += 1
            self.sheet_2.write_string(self.row_pos_2, 0, _('etiquetas analíticas'),
                                      self.format_header)
    ...
    def prepare_report_contents(self, data, acc_lines, filter):
        ...
        if filter.get('include_details', False):
            ...
            self.sheet.write_string(self.row_pos, 1, _('JRNL'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 2, _('Socio'),
                                    self.format_header)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        self.sheet = workbook.add_worksheet('Libro mayor')
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(0, 0, 0, 8, 'Libro mayor'+' - '+data[0]['company_id'][1], self.format_title)
