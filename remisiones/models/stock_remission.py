...
class StatePickingInherit(models.Model):
    ...
    def validate_client(self):
        ...
        if self.state in ['draft','assigned','partially_available']:
            ...
            if sale_order:
                if (sale_order.message_logistic == False and sale_order.message_sales == False and sale_order.message_residence == False) and self.origin[:1] == 'S':
    ...
    def parcial_remision(self):
        ...
        for line in remision:
            ...
            if not location:
                raise UserError(_('No se encontró la ubicación REMISION'))
            ...
            if not incoming_type:
                raise UserError(_('No se encontró el tipo de operación incoming para el almacén'))
            ...
            for rec in line.remisiones_line_id:
                ...
                # Remove price_list if it doesn't exist in Odoo 15
                # if hasattr(rec, 'price_list'):
                #     line_vals['price_list'] = rec.price_list
    ...
    def close_so(self):
        ...
        if 'Devoluciones' in self.picking_type_id.name and self.picking_type_id.code == 'incoming':
            ...
            if origin_picking.origin[:1] == 'S':
                ...
                origin_rem = self.env['remision'].search([('name','=',origin_picking.origin)], limit = 1)

                for line_so in origin_rem.remisiones_line_id:
                    qty_origin += line_so.cantidad

                if qty_done == qty_origin:
                    query = """UPDATE remision set state = 'closed' where id = {}""".format(origin_rem.id)
                    self.env.cr.execute(query)
