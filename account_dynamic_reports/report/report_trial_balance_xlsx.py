...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def prepare_report_filters(self, data, filter):
        ...
        if filter:
            # Date from
            self.sheet_2.write_string(self.row_pos_2, 0, _('Fecha de'),
                                    self.format_header)
    ...
    def prepare_report_contents(self, acc_lines, retained, subtotal, filter):
        ...
        self.sheet.write_string(self.row_pos, 0, _('Cuenta'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 1, _('Débito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 2, _('Crédito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 3, _('Balance'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 4, _('Débito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 5, _('Crédito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 6, _('Balance'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 7, _('Débito'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 8, _('Crédito'),
                                self.format_header)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record:
            data = record.read()
            self.sheet.merge_range(0, 0, 0, 10, 'Libro mayor'+' - '+data[0]['company_id'][1], self.format_title)
