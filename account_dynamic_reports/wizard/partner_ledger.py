...
class InsPartnerLedger(models.TransientModel):
    ...
    target_moves = fields.Selection(
        [('all_entries', 'Todos'),
         ('posted_only', 'Contabilizados')], string='Moviminetos',
    ...
    balance_less_than_zero = fields.Boolean(string='Con balance memor que 0')
    balance_greater_than_zero = fields.Boolean(string='Con balance mayor que 0')
    type = fields.Selection(
        [('receivable', 'Solo por cobrar'),
         ('payable', 'Solo por pagar')],
        string='Tipo de cuenta',required=False
    )
    initial_balance = fields.Boolean(
        string='Incluir balance inicial', default=True
    )
    reconciled = fields.Selection([('reconciled','Conciliados'),
                                   ('unreconciled','Solo no conciliados')],
    ...
    partner_category_ids = fields.Many2many(
       'res.partner.category', string='etiqueta de Cliente/proveedor',
    )
    ...
    def process_filters(self):
        ...
        if data.get('partner_category_ids', []):
            ...
            filters['categories'] = ['Todas']

        if data.get('target_moves') == 'all_entries':
            filters['target_moves'] = 'Todos'
        else:
            filters['target_moves'] = 'Contabilizados'
    ...
    def build_detailed_move_lines(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        sql = ('''
                ...
                JOIN account_journal j ON (l.journal_id=j.id)
                WHERE %s
                GROUP BY l.id,m.ref, l.partner_id, a.name, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.name, m.id, m.name, c.rounding, cc.id, cc.rounding, cc.position, c.position, c.symbol, cc.symbol, p.name
                ORDER BY %s
                OFFSET %s ROWS
                FETCH FIRST %s ROWS ONLY
    ...
    def action_xlsx(self):
        ...
        row_pos_2 = 0
        row_pos = 0
        sheet.merge_range(0, 0, 0, 8, 'Estado de cuenta' + ' - ' + data['company_id'][1], format_title)
        ...
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Mostar cuentas'), format_header)
        sheet_2.write(row_pos_2, 1, filter['display_accounts'], content_header)
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Reconciliado'), format_header)
        sheet_2.write(row_pos_2, 1, filter['reconciled'], content_header)
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Balance inicial'), format_header)
        ...
        # Partners
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Solcios'), format_header)
        p_list = ', '.join([lt or '' for lt in filter.get('partners')])
        sheet_2.write(row_pos_2, 1, p_list, content_header)
        # Partner Tags
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Etiqueta de Cliente/proveedor'), format_header)
        p_list = ', '.join([lt or '' for lt in filter.get('categories')])
        sheet_2.write(row_pos_2, 1, p_list, content_header)
        # Accounts
        row_pos_2 += 1
        sheet_2.write(row_pos_2, 0, _('Cuentas'), format_header)
        ...
        if filter.get('include_details', False):
            sheet.write(row_pos, 0, _('Fecha'),
                                    format_header)
            sheet.write(row_pos, 1, _('JRNL'),
                                    format_header)
            sheet.write(row_pos, 2, _('Cuenta'),
                                    format_header)
            # sheet.write(row_pos, 3, _('Ref'),
            #                         format_header)
            sheet.write(row_pos, 3, _('Movimineto'),
                                    format_header)
            sheet.write(row_pos, 4, _('Etiqueta de entrada'),
                                    format_header)
            sheet.write(row_pos, 5, _('Débito'),
                                    format_header)
            sheet.write(row_pos, 6, _('Crédito'),
                                    format_header)
            sheet.write(row_pos, 7, _('Balance'),
                                    format_header)
        else:
            sheet.merge_range(row_pos, 0, row_pos, 4, _('Cliente/proveedor'), format_header)
            sheet.write(row_pos, 5, _('Débito'),
                                    format_header)
            sheet.write(row_pos, 6, _('Crédito'),
                                    format_header)
            ...
            for line in account_lines:
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Balance inicial':
                            row_pos += 1
                            ...
                            sheet.write(row_pos, 7, float(sub_line.get('balance')),
                                                    line_header_light_initial)
                        elif sub_line.get('move_name') not in ['Balance inicial','Balance final']:
                            row_pos += 1
        ...
        report_id = self.env['common.xlsx.out'].sudo().create({'filedata': result, 'filename': 'EstadoDeCuenta.xls'})

        return {
            'type': 'ir.actions.act_url',
            'url': '/web/binary/download_document?model=common.xlsx.out&field=filedata&id=%s&filename=%s.xls' % (
                report_id.id, 'Estado de cuenta.xls'),
            'target': 'new',
        }
