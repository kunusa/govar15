...
class KsWooProductTemplate(models.Model):
    ...
    ks_language = fields.Many2one('res.lang', string="Language")
    ...
    def ks_manage_woo_product_template_import(self, instance, product_json_data, odoo_main_product=False,
        ...
        try:
            product_exist = False
            if product_json_data.get("sku"):
                product_exist = self.env['product.template'].search(
                    [('ks_woo_sku', '=', product_json_data.get("sku"))])
                if product_exist:
                    layer_exists = product_exist.ks_product_template.filtered(
                        lambda x: x.ks_wc_instance == instance)
                    if layer_exists:
                        product_exist = layer_exists
                    else:
                        odoo_main_product = product_exist
                        product_exist = False
                else:
            ...
            if product_json_data.get("type") == 'simple':
                if product_exist:
                    try:
                        if product_exist.ks_woo_product_type == product_json_data.get("type"):
                            main_product_data = self.ks_map_product_template_data_for_odoo(product_json_data, instance,
                                                                                           main_product=product_exist.ks_product_template)
                            ...
                            self.createTranslations(product_exist)
                            return product_exist
                        else:
                            if queue_record:
                    ...
                    try:
                        ...
                        self.createTranslations(woo_layer_product.ks_product_template)
                        self.createTranslations(woo_layer_product)
    ...
    def createTranslations(self,product):
        field_list = ['name','ks_woo_short_description','ks_woo_description']
        for name, fld in product._fields.items():
            if name in field_list:
                self.env['ir.translation'].insert_missing(fld, product)
    ...
    def ks_prepare_product_data_to_export(self, instance, product_template, product_type=False, product_config=False,
        ...
        """
        server_action = product_config.get("server_action") == True if product_config else False
        lang = instance.ks_language
        name = product_template.name
        translated_term = self.env['ir.translation'].search([('src', '=', name), ('lang', '=', lang.code)], limit=1)
        if translated_term and translated_term.state == 'translated':
            name = translated_term.value
        data = {
            "name": name,
            "type": product_type,
        }
        if server_action:
            if product_config['basic_info'] == True:
                desc = self.ks_woo_description
                short_desc = self.ks_woo_short_description
                if product_config.get('description'):
                    desc = product_config.get('description')
                if product_config.get('short_description'):
                    short_desc = product_config.get('short_description')
                translated_desc = self.env['ir.translation'].search(
                    [('src', '=', desc), ('lang', '=', lang.code)], limit=1)
                if translated_desc and translated_desc.state == 'translated':
                    desc = translated_desc.value
                translated_short_desc = self.env['ir.translation'].search(
                    [('src', '=', short_desc), ('lang', '=', lang.code)], limit=1)
                if translated_short_desc and translated_short_desc.state == 'translated':
                    short_desc = translated_short_desc.value
                data.update({
                    "description": desc or '',
    ...
    def create_woo_record(self, instance, odoo_product, export_to_woo=False, queue_record=False, product_config=False):
        ...
        try:
            ...
            if not product_exists:
                data = self.ks_map_prepare_data_for_layer(instance, odoo_product, product_config=product_config)
    ...
    def ks_map_product_template_data_for_layer(self, instance, product_json_data, odoo_main_product):
        ...
        translated_desc = self.env['ir.translation'].search(
            [('src', '=', product_json_data.get('description')), ('lang', '=', instance.ks_language.code)], limit=1)
        if not product_json_data.get('description') and not translated_desc:
            translated_desc = product_json_data.get('description')
        else:
            if translated_desc:
                translated_desc.write({
                    'value': product_json_data.get('description'),
                })
            else:
            #     translated_desc = self.env['ir.translation'].create({
            #         'src': product_json_data.get('description'),
            #         'value': product_json_data.get('description'),
            #         'lang': instance.ks_language.code,
            #         'name': 'ks.woo.product.template,ks_woo_description',
            #         'type': 'model',
            #         'state': 'translated',
            #     })
                translated_desc = product_json_data.get('description')
        ...
        translated_short_desc = self.env['ir.translation'].search(
            [('src', '=', product_json_data.get('short_description')), ('lang', '=', instance.ks_language.code)], limit=1)
        if not product_json_data.get('short_description') and not translated_short_desc:
            translated_short_desc = product_json_data.get('short_description')
        else:
            if translated_short_desc:
                translated_short_desc.write({
                    'value': product_json_data.get('short_description'),
                })
            else:
            #     translated_short_desc = self.env['ir.translation'].create({
            #         'src': product_json_data.get('short_description'),
            #         'value': product_json_data.get('short_description'),
            #         'lang': instance.ks_language.code,
            #         'type': 'model',
            #         'name': 'ks.woo.product.template,ks_woo_short_description',
            #         'state': 'translated',
            #     })
                translated_short_desc = product_json_data.get('short_description')
        data = {
            ...
            'ks_woo_description': translated_desc or '',
            'ks_woo_short_description': translated_short_desc or '',
            "ks_wc_instance": instance.id,
            "ks_product_template": odoo_main_product.id,
            "ks_company_id": instance.ks_company_id.id,
            "ks_language": instance.ks_language.id,
        }
        if layer_tags:
    ...
    def ks_map_product_template_data_for_odoo(self, json_data, instance, main_product=False):
        ...
        if main_product:
            translated_term = self.env['ir.translation'].search(
                [('src', '=', main_product.name), ('lang', '=', instance.ks_language.code)], limit=1)
            if not translated_term:
                translated_term = self.env['ir.translation'].create({
                    'src': main_product.name,
                ...
                translated_term = translated_term.value
            else:
                translated_term.write({
                    'value': json_data.get('name'),
                })
                translated_term = translated_term.value
        else:
            # translated_term = self.env['ir.translation'].create({
            #     'src': json_data.get('name'),
            #     'value': json_data.get('name'),
            #     'lang': instance.ks_language.code,
            #     'type': 'model',
            #     'name': 'product.template,name',
...
class KsProductTemplateInherit(models.Model):
    ...
    ks_woo_sku = fields.Char('Woocommerce SKU')
    ...
    def ks_manage_template_images(self, woo_product, template):
        # Primero verificar si hay imágenes en ks_image_ids
        ...
        # Si hay image_1920 pero ks_image_ids está vacío o no hay profile_image procesado, 
        # asegurar que la imagen principal se cree y procese
        if template.image_1920:
            # Verificar si ya existe una imagen procesada en WooCommerce para este template
            has_processed_images = woo_product.ks_wc_image_ids.filtered(
                lambda img: img.ks_wc_template_id == woo_product.id
            )
            
            # Si no hay imágenes procesadas o no hay profile_image, crear/actualizar la imagen principal
            if not has_processed_images or not woo_product.profile_image:
                # Buscar o crear un registro en ks.common.product.images para image_1920
                # Primero buscar si existe uno con profile_image_id
                if template.profile_image_id and template.profile_image_id.ks_template_id == template.id:
                    common_image = template.profile_image_id
                else:
                    # Crear un nuevo registro en ks.common.product.images
                    image_values = {
                        "sequence": 0,
                        "ks_image": template.image_1920,
                        "ks_name": template.name,
                        "ks_template_id": template.id
                    }
                    common_image = self.env["ks.common.product.images"].with_context(main_image=True).create(image_values)
                    # Actualizar profile_image_id
                    template.profile_image_id = common_image.id
                
                # Procesar la imagen para WooCommerce
                image_id = self.env['ks.woo.product.images'].ks_odoo_prepare_image_data(common_image,
                                                                                        template_id=woo_product.id,
                                                                                        variant_id=False)
                woo_product.profile_image = image_id.id
